...
fn main() {
    ...
    let  delind =cli.tick;
    let mut file = fs::OpenOptions::new().create(true).read(true).write(true).open("todo.txt").unwrap(); 
    let mut list=String::new();
    let mut towrite=String::new();
    let mut stringarr: Vec<String> = Vec::new();

    //Reading the file
    file.read_to_string(&mut list).expect("Error Reading the file");
   ...
   for i in 0..list.len(){
        ...
        match list.chars().nth(i){
            Some('\n') => {stringarr.push(taskk.clone());taskk.clear();},
            None =>{break;},
    ...
    match delind {
        ...
        None =>(),
        _ =>{panic!("Arguement out of bound")},
    ...
    //clear function
    if cli.clear == true {
        let mut file = fs::OpenOptions::new().read(true).write(true).truncate(true).open("todo.txt").unwrap(); 
    ...
    if flag {
        ...
        file.write(format!("{}", towrite).as_bytes()).expect("Error in Updating Tasks in File");
        println!("{}",towrite);
    }
    else{
